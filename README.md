...
本项目采用“监测模式”：
1. PubMed 以 `Publication Date [dp]` 为主（最近发表优先）。
2. 默认核心窗口为最近 14 天（therapy/ddi 30 天，reviews/guidelines 60 天）。
3. 只输出历史未见的新记录。
4. 双层检索策略：
...
## 3. 最近发表 vs 最近入库（`[dp]` vs `[crdt]`）

本项目当前默认采用“最近发表优先”策略：
- `[dp]`/`[pdat]`：更关注“最近发表”的文章；
- `[crdt]`：更关注“最近进入数据库”的记录。
...
- 邮件通知相关（可不填，不填会自动跳过）：
  ...
  - `EMAIL_PASSWORD`
  - `EMAIL_TO`
- `STRICT_RECENT_PUBLICATION=true`（只保留最近发表记录，所有来源统一按 publication_date 后置过滤）
...
- PubMed 可能触发接口限流（HTTP 429）；本项目已加入请求降速（间隔 sleep）与重试退避（2s/5s/10s）以提高稳定性。
